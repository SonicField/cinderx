...
# ── Suite: specialisation — adaptive specialisation correctness tests ─────

# These tests verify that JIT-compiled specialised opcodes (LOAD_ATTR,
# STORE_ATTR, FOR_ITER, BINARY_OP) produce correct results, including
# under deoptimisation (class modification, subclass, polymorphic access).
#
# Test files live alongside test_cinderx.sh as test_<name>.py.
# Each is a standalone script: exit 0 = pass, non-zero = fail.

SPECIALISATION_TESTS=(
...
run_specialisation() {
    local total=${#SPECIALISATION_TESTS[@]}
    local pass=0
    local fail=0
    local skip=0
    local idx=0
    ...
    echo ""
    echo -e "${BOLD}=== Specialisation: Adaptive Opcode Tests ($total tests) ===${RESET}"

    for entry in "${SPECIALISATION_TESTS[@]}"; do
        idx=$((idx + 1))
        local file="${entry%%:*}"
        local desc="${entry#*:}"
        local path="$SCRIPT_DIR/$file"

        printf "  [%d/%d] %-45s " "$idx" "$total" "$desc"
        ...
        local output
        output=$(timeout 120 "$PYTHON" "$path" 2>&1)
        local rc=$?
        ...
        if [ $rc -eq 0 ]; then
            ...
            if echo "$output" | grep -q "^SKIP"; then
                ...
                # Count passes from output (look for "X passed" pattern)
                local test_pass
                test_pass=$(echo "$output" | grep -oE "[0-9]+ passed" | grep -oE "[0-9]+" || echo "1")
                echo -e "${GREEN}PASS${RESET} ($test_pass passed)"
                pass=$((pass + test_pass))
            fi
        else
            ...
            fail=$((fail + 1))
            # Save failure log
            echo "$output" > "$RESULTS_DIR/cinderx_fail_specialisation_${file%.py}.log"
        fi
    done

    record_suite "Specialisation Tests" "$pass" "$fail" "$skip" "0"
}
