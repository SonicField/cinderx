#!/bin/bash
# CinderX Test Runner — aarch64 JIT development
#
# Runs all CinderX test suites on devgpu004 and produces a summary report.
# Designed for the cmake/pip-editable build on aarch64.
#
# Usage:
...
OPCODE_SRC="$PYTHONLIB/opcodes/3.12/opcode.py"
OPCODE_DST="$PYTHONLIB/cinderx/opcode.py"
...
case "$ARG" in
    ...
    help|-h)
        echo "Usage: $0 [all|jit|runtime|compiler|TESTNAME]"
        echo ""
        echo "  all       Run all test suites (default)"
        echo "  jit       Run JIT test suites only"
        echo "  runtime   Run runtime test suites only"
        echo "  compiler  Run compiler test suites only"
...
if [ ${#ERROR_SUITES[@]} -gt 0 ]; then
    ...
    for s in "${ERROR_SUITES[@]}"; do echo "  - $s"; done
    echo ""
fi
