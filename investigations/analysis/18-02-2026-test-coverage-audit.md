...
- Strict counting: 36/41 PASS (test_cinderjit 171/172 with 1 error → FAIL under strict rules)
...
**D93679174 APPLIED** (07:00Z 19 Feb — commit 22c40cac, @testkeeper):
- CinderX regression suite: 36/41 PASS — baseline maintained
- Import-time auto-compile SEGFAULT: NOT fixed by D93679174 alone
- Safe sequencing (torch import first, auto-compile after): still works
...
**Upstream fixes verified present (19 Feb ~06:25Z, @testkeeper on devgpu004):**
Two of three upstream ARM64 fixes are already in our build at d50fa6ad:
- D92979437 (kLea codegen): autogen.cpp leaIndex() uses case 0/1/2/3 (fixed). Already present.
- D93621746 (double spill): postalloc.cpp insertMoveToMemoryLocation() uses DataType::k64bit for FP branch (fixed). Already present.
- D93679174 (primitives w1/w0): NOT present at d50fa6ad — gen_asm.cpp uses w2 (should be w1), autogen.cpp uses umov (should be fmov). **Committed as 22c40cac by @testkeeper (19 Feb ~07:00Z).** All 4 edits applied: autogen.cpp umov→fmov, gen_asm.cpp mov(w2,w0)→mov(w1,w0), movi(d0,1)→fmov(d1,1.0), mov(w2,1)→mov(w1,1), cmp(w2,0)→cmp(w1,0).
CORRECTION (19 Feb ~06:35Z): Initial verification incorrectly reported D93679174 as already present. The error was caused by checking on-disk files (which had uncommitted fixes) instead of the committed git version. Always use `git show COMMIT:path` to verify committed state.

**D93679174 test results (19 Feb ~07:00Z, @testkeeper):**
- CinderX regression suite: 36/41 PASS — matches d50fa6ad baseline exactly. No regression.
- Import-time auto-compile (cinderjit.auto() BEFORE torch import): still SEGFAULT (signal 11). D93679174 alone does NOT fix the import-time crash.
- Safe sequencing (torch import FIRST, then cinderjit.auto()): still works. Post-import auto-compile is safe.
- Conclusion: D93679174 is a necessary correctness fix but NOT the root cause of the import-time SEGFAULT. Additional upstream fixes are needed.

### Gap 8: PurePath.__hash__ corruption under auto-compile (NEW — 19 Feb 06:14Z)

When cinderjit.auto() compiles PurePath.__hash__ (from stdlib pathlib), it corrupts PosixPath._hash attribute access. This breaks pytest's internal path handling. The JIT mishandles cached_property or descriptor __get__ protocol during auto-compilation of stdlib classes.

**Root cause:** JIT auto-compiling stdlib classes that rely on descriptor protocol / cached_property. When PurePath.__hash__ is JIT-compiled, the descriptor mechanism for _hash (a cached_property) is broken.

**Workaround:** Use force_compile on specific functions rather than blanket auto-compile. Confirmed working for PyTorch nn.Module dispatch (4 functions force-compiled, exact match vs interpreted).

**Impact:** Auto-compile cannot be used with low thresholds (100) — it compiles stdlib before reaching torch code, hitting this bug. Higher thresholds (1000+) may avoid it but are untested.

### Gap 9: aarch64 JIT exception handling — LOAD_ATTR in try/except crashes (NEW — 19 Feb 07:03Z)

**Root cause CONFIRMED by minimal reproducer (19 Feb ~07:05Z, @testkeeper on devgpu004 build 22c40cac):**
...
# Interpreted: returns "caught" (correct)
# force_compile(test_dot) → True
# JIT execution: SEGFAULT (signal 11)
```

The aarch64 JIT successfully compiles functions with try/except blocks, but when a LOAD_ATTR opcode inside a try block raises AttributeError, the JIT-compiled code SEGFAULTs instead of transferring control to the except handler.

**Why this explains ALL auto-compile crashes:**
- Import-time SEGFAULT: `inspect._signature_from_callable` uses `try: obj.__signature__ except AttributeError` pervasively
- PurePath.__hash__ corruption: may also involve LOAD_ATTR in exception paths
- The 36/41 CinderX baseline is GREEN because existing tests don't heavily exercise LOAD_ATTR-in-try/except via force_compile

**This is NOT related to D93679174** (primitive return registers). It is a separate, fundamental codegen bug in the exception dispatch mechanism for the aarch64 JIT.

**Falsifier:** If the minimal reproducer above returns "caught" after a fix, the exception handling bug is resolved. If it still crashes, the fix is incomplete.

**Impact:** CRITICAL — blocks all auto-compile usage and any force_compile of functions containing try/except + attribute access.

**Coverage analysis (19 Feb ~07:35Z, @testkeeper):**

The existing `test_jit_exception.py` (15 tests, all pass) tests exceptions exclusively from **CALL** and **RAISE** bytecodes — never from data-access opcodes. This is why the JIT exception handling appears functional despite the missing exception table support: CALL bytecodes emit CheckExc in the HIR, so exceptions from called functions are caught correctly. LOAD_ATTR, BINARY_SUBSCR, STORE_ATTR etc. do NOT emit CheckExc, so exceptions from these opcodes are not caught.

**Root cause refined (19 Feb ~07:30Z):** ~~Initially hypothesised as "HIR builder doesn't parse exception tables".~~ **CORRECTED (19 Feb ~07:45Z):** Empirical testing falsified the general hypothesis. BINARY_SUBSCR, STORE_ATTR, DELETE_ATTR, LOAD_GLOBAL, STORE_SUBSCR, STORE_ATTR (cached) all correctly catch exceptions in try/except in JIT-compiled code. Only LOAD_ATTR fails. The LIR generator has an exemption list at generator.cpp:3551 that skips `emitExceptionCheck` for certain opcodes. `kLoadAttrCached` is in this list but does NOT handle its own exception — its `appendCallInstruction` has no Guard. All other exempted opcodes (kStoreAttr, kStoreAttrCached, kStoreSubscr, etc.) correctly handle their own exceptions. **Fix: remove `case Opcode::kLoadAttrCached:` from the exemption list.** This is a one-line change.

**PROVENANCE (19 Feb ~07:52Z, @testkeeper):** The exemption was added by commit 3c4ff942 ("Option D: LIR inline type guard"). Option D added an inline fast-path that was supposed to handle its own exceptions. Commit 8af4dc49 reverted the fast-path but did NOT remove the exemption. This is OUR bug from the Option D revert, not a pre-existing CinderX issue. Upstream fbsource does not have kLoadAttrCached in the exemption list.

**Test coverage design:** See `19-02-2026-exception-coverage-design.md` — 5 groups of tests covering LOAD_ATTR, BINARY_SUBSCR, STORE_ATTR, DELETE_ATTR, mixed patterns, and auto-compile integration. All follow the existing `test_jit_exception.py` pattern using `@failUnlessJITCompiled`.
...
- If applying upstream fixes D92979437, D93621746, D93679174 changes any codegen output: our build does NOT already have them. Verified: D92979437 and D93621746 are present in d50fa6ad source. D93679174 is NOT present — verified by `git show d50fa6ad:cinderx/Jit/codegen/gen_asm.cpp` showing w2 and movi(d0) (broken versions). Initial grep-based check was falsified by uncommitted working tree changes.
