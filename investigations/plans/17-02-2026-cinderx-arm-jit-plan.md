...
## Current State (updated 18 Feb 2026 07:30 UTC — Phase 5e spec GATE GREEN, awaiting implementation)

- **PHASE 4: COMPLETE** — 15/15 CPU modules pass, 8/8 GPU modules pass, ZERO regressions. Committed as 68126095.
- **PHASE 4c: DEPLOYED** — Generator deopt guard (sha 3c67492d). Prevents JIT compilation of generators/coroutines/async generators on aarch64 via 3 guard sites in pyjit.cpp. 24/24 tests pass. ZERO regressions.
- **PHASE 5c ATTEMPTED AND REVERTED** — originalFP indirection approach (sha 6be29706) crashed on resume (offset mismatch). Reverted to 3c67492d by team consensus.
- **PHASE 5d (2-file fix, sha 52efadf8): GATE RED** — Two failure modes found:
  1. **HANG** — frame.cpp getIP() aarch64 frame chain walk infinite loop: walks stack FP chain looking for heap address (generator's FP), never finds it → infinite loop
  2. **SIGBUS** — writer-side crash on force_compile with many-arg generators: STR to [FP + saved_ip_fp_offset] causes SIGBUS
  - Testkeeper debugging SIGBUS on devgpu004 with GDB
  - Claude identified HANG cause: frame chain walk designed for stack addresses, fails with heap FP
- **DECISION:** Revert to 3c67492d and implement full 7-file GenDataFooter.savedIP approach (Alex's chosen approach). FP offset RESOLVED: FP = GenDataFooter* (struct start). Gate GREEN.
...
- **Bug #4 (frame.cpp:138): FIXED — SP-relative saved-IP slot [SP,#8] (sha 68126095)**
  ...
  - **Validated:** Clean rebuild from committed source (md5 18c00184) verified via JIT disassembly: emits `STR x12, [SP, #8]` (f90007ec) and `STR XZR, [SP, #8]` (f90007ff). Passes test_type_promotion (423/423 CPU), polymorphic vectorcall (3/3 with env var), dynamic vectorcall (all tests).
  - **CRITICAL FINDING — .so PROVENANCE:** The old deployed .so (md5 81434f44, labelled "68126095-backup") was NOT built from committed code. JIT disassembly showed FP-relative `STR x12, [FP]` (f90003ac), not SP-relative `STR x12, [SP, #8]` (f90007ec). All prior 15/15 CPU and 8/8 GPU baselines were against this FP-relative .so. Clean rebuild (md5 18c00184) confirmed correct SP-relative instructions. Both variants pass PyTorch tests with env var activation.
  - **Latent risks (post-Phase 4):** kVectorcallArgsOffset=1 overlap with [SP,#8] — theoretical overlap exists in committed code, but EMPIRICALLY NOT TRIGGERED with env var activation (PYTHONJITCOMPILATIONTHRESHOLD). 3/3 PASS polymorphic vectorcall with committed .so. cinderjit.auto() SIGBUS (3/3) is the pre-existing auto() bug, not the overlap. PYTHONJITALL=1 crash is separate pre-existing aarch64 JIT bug.
  - **FP-relative fix (Phase 5 prep):** 8-file changeset preserved in `git stash@{0}` on devgpu004, labelled "Phase 5: FP-relative saved-IP slot". Uses `ptr_resolve(FP, saved_ip_fp_offset, scratch)` to write saved-IP in header/spill region near FP, eliminating vectorcall overlap. Source-level review: COMPLETE and internally consistent (all 4 write sites, 2 getIP readers, 2 getUnitState callers, 1 zero-init updated). First build CRASHED (SIGSEGV on vectorcall tests) — most likely cause: incremental build that picked up partial changes. Phase 5 action: apply stash, make clean && make (full clean build), GDB verify emitted instructions, test.
...
## Phase 5: Generator Fix — Remove Deopt Guard

**Terminal Goal:** Run PyTorch faster on aarch64 using CinderX JIT. The PR is a nice-to-have, not the primary goal.

**Owner:** testkeeper (implementation on devgpu004), gatekeeper (gate review), claude (analysis)
**Falsifier:** Generators/coroutines/async generators JIT-compile and execute correctly on aarch64. All 24 generator-specific tests pass. 15/15 CPU modules pass. No regressions.

### Phase 5a: PR Preparation (SECONDARY)
- Extract portable fixes from our work for upstream PR
- Separate from generator fix — can proceed in parallel once Phase 5c done

### Phase 5b: Analysis (COMPLETE)
- PR #13 analysis complete (18-02-2026-pr13-analysis.md)
- Kevin Newton confirmed PR #13 is "AI slop" — basicsize bug would corrupt generator objects
- Our work is independent of PR #13

### Phase 5c: Generator Fix — Approach Selected (GenDataFooter.savedIP)
**Alex's decision (18 Feb 2026):** Use GenDataFooter.savedIP field approach — "makes fewer assumptions."

**Design (7-file approach, from testkeeper):**
1. `gen_data_footer.h` — Add `uint64_t savedIP{0}` field to GenDataFooter
2. `environ.h` — Add `bool is_generator{false}` field
3. `gen_asm.cpp` — Set env_.is_generator before generateCode; zero-init savedIP in prologue
4. `autogen.cpp` — For generators on aarch64, STR return addr to `[footer + offsetof(GenDataFooter, savedIP)]` (REPLACES existing stack-relative STR for generators)
5. `gen_asm_utils.cpp` — Same as (4)
6. `frame.cpp` — For running generators on aarch64, read footer->savedIP instead of frame chain walk
7. `pyjit.cpp` — Remove 3 deopt guard sites (kCoFlagsAnyGenerator checks)

**Why the 2-file shortcut failed (Phase 5d, sha 52efadf8):**
- Writer: STR to [FP + negative offset] causes SIGBUS for some generator frame layouts
- Reader: frame.cpp getIP() frame chain walk designed for stack addresses, infinite loops with heap FP
- Both bugs stem from treating generators' heap-allocated GenDataFooter FP as a stack FP

**Current status:** Phase 5d (2-file fix) GATE RED. Testkeeper debugging SIGBUS. Decision: revert to 3c67492d and implement full 7-file approach.

**Iteration Budget (per Pythia #31):** ONE patch iteration max after initial 7-file implementation. If the approach requires more than one corrective patch, STOP and ship Phase 4c as-is. The generator fix is the right thing to do but not at infinite cost.

**Pre-Commit Checklist (MANDATORY):**
- [ ] Full 24/24 generator crash vector tests pass with generators JIT-compiled
- [ ] 15/15 CPU module tests pass
- [ ] GDB verification of FP→GenDataFooter mapping
- [ ] Gatekeeper gate GREEN (independent verification, not delegated reasoning)
- [ ] No coding before gate GREEN on design spec

**Phase 4c Performance Measurement (PRIORITY):**
Before starting Phase 5e coding, measure Phase 4c JIT ON vs JIT OFF on one PyTorch benchmark. If <2% speedup, reassess whether generator fix is the right priority vs other optimisation work.

**Detailed Implementation Spec (Phase 5e — corrected by supervisor):**

```
FILE 1: gen_data_footer.h
  Add after code_rt (or after frame_header if ENABLE_LIGHTWEIGHT_FRAMES):
    uintptr_t savedIP{0};

FILE 2: environ.h
  Add to Environ class:
    bool is_generator{false};

FILE 3: gen_asm.cpp
  In NativeGenerator::generateCode(), before generatePrologue():
    env_.is_generator = (code->co_flags & kCoFlagsAnyGenerator);

FILES 4+5: autogen.cpp, gen_asm_utils.cpp (CRITICAL: REPLACE, not supplement)
  In emitCall/translateCall aarch64 path — CONDITIONAL store:

  FP→GenDataFooter mapping: RESOLVED (independently verified by theologian 18 Feb).
  Source evidence (read directly on devgpu004):
    gen_asm.cpp:~2565  jit_data_r = gen->gi_jit_data  (LDR from PyGenObject)
    gen_asm.cpp:~2595  as_->mov(arch::fp, jit_data_r)  // comment: "X29 = gen->gi_jit_data"
    gen_data_footer.h  "gi_jit_data points above the top of the spill data
                        (i.e. at the start of the footer)"
  Layout: [FP+0]=linkAddress, [FP+8]=returnAddress, [FP+16]=originalFramePointer
  Therefore: offsetof(GenDataFooter, savedIP) is the correct offset from FP.
  The resume code already uses raw offsetof(GenDataFooter, yieldPoint) from FP,
  confirming this is the established pattern.
  GDB confirmation recommended as belt-and-suspenders (testkeeper's GDB session).

    #if defined(__aarch64__)
    auto after_call = as_->newLabel();
    as_->adr(scratch, after_call);
    if (env_.is_generator) {
      // Generator: write to footer savedIP field (positive offset, always aligned)
      as_->str(scratch, a64::ptr(a64::x29, offsetof(GenDataFooter, savedIP)));
    } else {
      // Non-generator: existing stack-relative write
      as_->str(scratch, a64::ptr(a64::x29, saved_ip_fp_offset));
    }
    // ... BL/BLR call ...
    as_->bind(after_call);
    #endif
...
FILE 6: frame.cpp
  In getIP(), generator running case on aarch64:
    if (isGeneratorFrame(frame)) {
      auto footer = jitGenDataFooter(gen);
      if (footer->yieldPoint == nullptr) {
        #if defined(__aarch64__)
        return footer->savedIP;  // Direct read, no frame chain walk
        #else
        frame_base = footer->originalFramePointer;
        // Falls through to x86 saved-IP read
        #endif
      } else {
...
FILE 7: pyjit.cpp
  Remove 3 kCoFlagsAnyGenerator deopt guard sites.
```

### Phase 5d: vLLM Performance Testing
- Alex setting up vLLM test system
- Not started — blocked on Phase 5c
...
| **claude** | Technical architect — 7-file spec design, FP offset analysis, code review, PR preparation |
...
| Generator heap FP incompatible with stack frame chain walk | Blocks Phase 5 generator fix | **NEW — ACTIVE** — frame.cpp getIP() walks stack FP chain looking for heap address → infinite loop. 2-file fix insufficient. 7-file GenDataFooter.savedIP approach avoids frame chain walk entirely. |
| Process violations (unauthorised commits) | Source-binary mismatch, lost trust | **MITIGATED** — 6be29706 committed without gate review, caused source-binary mismatch caught by gatekeeper. Rule: ONE person modifies source, ONE build, ONE test. Always build from source if in doubt. |
| PR #13 (external) semantic incompatibility | Corrupts generator objects if merged | **MONITORED** — PR #13 removes sizeof(GenDataFooter*) from basicsize without updating jitGenDataFooterPtr(). Kevin (CinderX maintainer) called it "AI slop". Alex to ping Kevin. No impact on our Phase 4c/5c. |
