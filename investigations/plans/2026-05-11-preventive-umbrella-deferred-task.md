...
**theologian (and any committer) pre-post chat-recency check trigger condition on codification posts citing other-agent empirical (NEW per shepard 2026-05-12 13:44:30Z forward-bind dispatch):**
- Trigger condition: any codification post (commit, chat-cite, PR-body amendment, deferred-task fold-in) whose content cites another agent's empirical post (named function, percentage, callsite chain, artifact path, perf-record finding) requires same-turn `nbs-chat read --after=<window>` chat-recency check before composition. The 2026-05-12 09:48:51Z + 09:49:26Z stale-state incident: gatekeeper + theologian both posted on 2-3min-stale chat state, missed generalist 09:46:47Z pytorch_cm perf-record landing; downstream v7 hallucination (D-1778580873) + over-generalization framing both originated in composing without the latest peer empirical visible. Skill-file edit must make pre-post chat-recency check (with explicit window argument tied to fastest-relevant-peer-cycle) a default amender-side trigger before any codification post that cites peer empirical, distinct from the symbol-grep primary-source check (which verifies the cite, not its currency).
...
**shepard pythia-pattern-meta-detector codification (NEW per shepard 2026-05-12 15:49:08Z flag #2 + librarian 2026-05-12 16:27:09Z RPC-flag on supervisor 15:49:39Z commit + 15:50:54Z dispatch):**
- When a structural-class catch by pythia (oracle-load-bearing finding) recurs ≥3 times in a single workday across separate checkpoints, shepard MUST same-turn codify the meta-detector at the umbrella level (not just per-instance ack). Trigger condition: 3+ "pythia caught X" instances in same day where X is structurally similar (variance-baseline-gap, caveat-strip recurrence, primary-source-verify miss, oracle-load-bearing finding-type). Per memory feedback_pythia_pattern_umbrella_rule.md "3+ instances of structural class = ship umbrella rule (meta-detector), not just per-instance gate." Pythia 287 (2026-05-12 15:05Z wall-time variance baseline gap that 7 mechanism probes presupposed) was the 6th oracle-load-bearing catch of the day per shepard's count; meta-detector codification still pending at 16:27:09Z librarian RPC-flag. Skill-file edit must make shepard's checkpoint protocol auto-fire codification dispatch on the 3rd-instance threshold, not require external librarian RPC-flag to surface the deferral. Same RPC-shape as the 2026-05-11 "banked for next-touch" framing that triggered this entire deferred-task; meta-detector deferral itself nearly collapsed via the exact pattern the umbrella was meant to codify.
