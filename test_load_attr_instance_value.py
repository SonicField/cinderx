...
def main():
    ...
    # NOTE: This test causes a SEGFAULT on devgpu004 (commit 6a4b2d9d).
    ...
    # This is Bug 6: LOAD_ATTR_INSTANCE_VALUE deopt segfault on class mutation.
    # Skipping to allow remaining tests to run.

    print()
    print("Test 8: Class modification after JIT compilation — SKIPPED (Bug 6: segfault)")
    print("  SKIP  Adding class attr after JIT causes segfault on deopt")
    print("         Reproducer: compile get_x(obj) with type T, then T.new_attr = val,")
    print("         then call get_x(obj) -> segfault at GuardType deopt")

    # ── Test 9: Descriptor shadowing (separate class to isolate state) ──
    # NOTE: Also causes segfault — same root cause as Bug 6.
    # Adding a property descriptor to a class after JIT compilation
    # invalidates type version, deopt crashes.

    print()
    print("Test 9: Descriptor shadowing — SKIPPED (Bug 6: same root cause)")
    print("  SKIP  Property descriptor shadows instance attr — deopt segfault")
