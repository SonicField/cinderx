...
ASMJIT_API Error CodeHolder::bindLabel(const Label& label, uint32_t toSectionId, uint64_t toOffset) noexcept {
  ...
  while (link) {
    ...
    if (relocId != Globals::kInvalidId) {
      ...
      // AArch64 tbz/tbnz, cbz/cbnz, and b.cond relaxation: 8-byte region with
      // conditional + unconditional branch. Both use bit inversion for the condition
      // and encode the offset in the same position (bits [N:5]).
      if (link->format.type() == OffsetType::kAArch64_TestBranch ||
        ...
        // b.cond uses bit 0 for condition inversion, tbz/cbz use bit 24.
        uint32_t inversionMask = (link->format.type() == OffsetType::kAArch64_CondBranch) ? 1u : (1u << 24);
